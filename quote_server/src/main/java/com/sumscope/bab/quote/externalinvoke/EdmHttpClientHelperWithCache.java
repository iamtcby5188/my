...
public class EdmHttpClientHelperWithCache {
    ...
    public Map<String, IamProvinceAndIdDTO> getIamProvinceAndIdDTO() {
        try {
            List<IamProvinceAndIdDTO> cn = edmHttpClientWithCache.getIamProvinceAndIdDTOByCountryCode("CN");
            Map<String, IamProvinceAndIdDTO> map = new HashMap<>();
            for (IamProvinceAndIdDTO dto : cn) {
    ...
    public IamProvinceAndIdDTO getIamProvinceAndIdDTO(String provinceCode) {
        try {
            IamProvinceAndIdDTO iamProvinceAndIdDTOByCode = edmHttpClientWithCache.getIamProvinceAndIdDTOByCode(provinceCode);
            if (iamProvinceAndIdDTOByCode == null) {
                LogStashFormatUtil.logWarrning(logger,"按代码从IAM系统获取省份信息失败！code: " + provinceCode);
            }
            return iamProvinceAndIdDTOByCode;
        } catch (ExternalInvocationFailedException e) {
            LogStashFormatUtil.logWarrning(logger,"按代码从IAM系统获取省份信息失败！" + e);
        }
        return null;
    }
}
